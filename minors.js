...
export async function fetchMinorsFromFirestore(userId) {
    try {
        ...
        checkboxContainer.innerHTML = ''; // ここでチェックボックスをクリア
        ...
        querySnapshot.forEach((doc) => {
            ...
            if (minor.userId === userId) { // ユーザーIDでフィルター
                ...
                displayCheckbox(minor); // チェックボックスを表示
            }
        });
    } catch (error) {
...
// チェックボックスを表示する関数
function displayCheckbox(minor) {
    const labels = getLabels(); // 現在の言語に応じたラベルを取得
    const checkboxContainer = document.getElementById('minorCheckboxContainer');
...
export function addMinorEventListener() {
    const addButton = document.getElementById('addMinorInfoButton');

    // 既存のイベントリスナーを削除（重複登録を防ぐため）
    addButton.removeEventListener('click', handleAddMinor);
    
    // 新しいイベントリスナーを追加
    addButton.addEventListener('click', handleAddMinor);
}

async function handleAddMinor() {
    ...
    // チェックボックスを生成
    displayCheckbox(minor);

    // 登録された未成年者リストに追加
    const infoList = document.getElementById('infoList');
    const listItem = document.createElement('li');
    listItem.textContent = `${getLabels().minorLabel} ${name}, ${getLabels().ageLabel} ${age}`;
